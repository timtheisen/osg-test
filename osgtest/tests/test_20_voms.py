...
class TestVOMS(unittest.TestCase):
    ...
    def test_02_install_voms_certs(self):
        if core.rpm_is_installed('voms-server'):
    ...
    def test_05_add_local_admin(self):
        if core.missing_rpm('voms-admin-server', 'voms-mysql-plugin'):
            return
        host_dn, host_issuer = core.certificate_info(TestVOMS.__hostcert)
    ...
    def test_13_wait_for_voms_admin(self):
        if core.missing_rpm('voms-admin-server'):
            return
        line, gap = core.monitor_file(TestVOMS.__vomsadmin_logfile,
                                      TestVOMS.__vomsadmin_log_start,
    ...
    def test_50_config_mkgridmap(self):
        ...
        contents = 'group vomss://%s:8443/voms/%s %s\n' % \
            (socket.getfqdn(), TestVOMS.__voname, core.options.username)
        config = open(TestVOMS.__mkgridmap_conf, 'w')
        config.write(contents)
        config.close()
        core.syspipe(('cat', TestVOMS.__mkgridmap_conf))
    ...
    def test_97_remove_vo(self):
        ...
        status, stdout, stderr = core.syspipe(command)
        fail = core.diagnose('Drop MYSQL VOMS database', status, stdout, stderr)
        self.assertEqual(status, 0, fail)
